...
export class Followed {
 ...
 constructor(
   ...
   this.title = 'Seguidores de ';                    // Título de la página
 ...
 ngOnInit(): void {
   ...
   console.log('Followed.Component cargado!!');
   this.actualPage();
 }
   ...
   this._followService.getFollowed(this.token, user_id, page).subscribe(
     response => {
       ...
       if (!response.followedUsers) {
         ...
         this.followed = response.followedUsers; // Almacenar la lista de usuarios
         this.follows = response.users_follow_me; //Almacenar lista de usuarios que sigue 
